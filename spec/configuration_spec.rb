...
describe PlugThumbnailer::Configuration do

  let(:instance) { described_class.new }

  it { expect(instance.redirect_limit).to         eq(3) }
  it { expect(instance.user_agent).to             eq('plug_thumbnailer') }
  ...
  describe '.config' do
    ...
    it { expect(PlugThumbnailer.config).to be_a(described_class) }
  ...
  describe '.configure' do
    ...
    before do
      allow(PlugThumbnailer).to receive(:config).and_return(instance)
    end

    context 'when block given' do

      it 'yields' do
        expect(PlugThumbnailer).to receive(:configure).and_yield(instance)
        PlugThumbnailer.configure {|config|}
      end
    ...
    context 'when no block given' do
      ...
      it 'does nothing' do
        expect(PlugThumbnailer.configure).to be_nil
      end
