...
describe PlugThumbnailer::Processor do

  let(:page)      { ::PlugThumbnailer::Page.new(url, {}) }
  let(:instance)  { described_class.new }
  let(:url)       { 'http://foo.com' }
  before do
    allow(PlugThumbnailer).to receive(:page).and_return(page)
  end
  ...
  describe '#call' do
    ...
    context 'when redirect_count is greater than config' do
      ...
      it { expect { instance.call(url, 10) } .to raise_error(PlugThumbnailer::RedirectLimit) }
    ...
    context 'on http error' do
      ...
      it { expect { action }.to raise_error(PlugThumbnailer::HTTPError) }
  ...
  describe '#with_valid_url' do
    ...
    context 'when not valid' do
      ...
      it { expect { instance.send(:with_valid_url) }.to raise_error(PlugThumbnailer::BadUriFormat) }
