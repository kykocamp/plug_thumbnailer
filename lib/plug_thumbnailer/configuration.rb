...
module PlugThumbnailer
  ...
  #
  # @return [PlugThumbnailer::Configuration] a configuration instance.
  def self.config
  ...
  def self.configure
    ...
    #
    # @return [PlugThumbnailer::Configuration]
    def initialize
      @redirect_limit         = 3
      @user_agent             = 'plug_thumbnailer'
      @verify_ssl             = true
      @http_open_timeout      = 5
      @http_read_timeout      = 5
      @blacklist_urls         = [
      ...
      @graders                = [
        ...
        ->(description) { ::PlugThumbnailer::Graders::Position.new(description, weigth: 3) },
