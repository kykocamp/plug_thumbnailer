...
module PlugThumbnailer
  class Processor < ::SimpleDelegator
    ...
    def call(url = '', redirect_count = 0, headers = {})
      ...
      raise ::PlugThumbnailer::RedirectLimit if too_many_redirections?
      ...
      raise ::PlugThumbnailer::HTTPError.new(e.message)
    end

    private

    def with_valid_url
      raise ::PlugThumbnailer::BadUriFormat unless valid_url_format?
    ...
    def perform_request
      ...
      raise ::PlugThumbnailer::FormatNotSupported.new(response['Content-Type']) unless valid_response_format?(response)
